...
apl_decompress:
			   ...
			   move.l #32000,a2     ; load 32000 offset constant
			   move.l #1280,a3      ; load 1280 offset constant
			   move.l #128,a4       ; load 128 offset constant
...
.next_token:   bsr .get_bit       ; read 'literal or match' bit
               bcc .literal       ; if 0: literal

               bsr .get_bit       ; read '8+n bits or other type' bit
               bcs .other_match   ; if 11x: other type of match

               bsr .get_gamma2    ; 10: read gamma2-coded high offset bits
               sub.l d2,d0          ; high offset bits == 2 when LWM == 3 ?
               bcc .no_repmatch   ; if not, not a rep-match

               bsr .get_gamma2    ; read repmatch length
               bra .got_len       ; go copy large match
...
.no_repmatch:  lsl.l #8,d0          ; shift high offset bits into place
               ...
               bsr .get_gamma2    ; read match length
               cmp.l a2,d3          ; offset >= 32000 ?
               bge .inc_by_2      ; if so, increase match len by 2
               cmp.l a3,d3          ; offset >= 1280 ?
               bge .inc_by_1      ; if so, increase match len by 1
               cmp.l a4,d3          ; offset < 128 ?
               bge .got_len       ; if so, increase match len by 2
...
.copy_match:   move.b (a6)+,(a1)+   ; copy matched byte
               ...
               bra .next_token    ; go decode next token

.other_match:  bsr .get_bit       ; read '7+1 match or short literal' bit
               ...
               bra .got_len       ; go copy match

.short_match:  moveq #0,d0          ; clear short offset before reading 4 bits
               bsr .get_dibits    ; read a data bit into d0, one into carry
               addx.b d0,d0         ; shift second bit into d0
               bsr .get_dibits    ; read a data bit into d0, one into carry
               addx.b d0,d0         ; shift second bit into d0
               beq .write_zero    ; if offset is zero, write a 0
...
.write_zero:   move.b d0,(a1)+      ; write matched byte or 0
               bra .after_lit     ; set LWM flag and go decode next token
...
.gamma2_loop:  bsr .get_dibits    ; read data bit, shift into d0
                                    ; and read continuation bit
               bcs .gamma2_loop   ; loop until a 0 continuation bit is read
               rts

.get_dibits:   bsr .get_bit       ; read bit
               addx.l d0,d0         ; shift into d0
                                    ; fall through
.get_bit:      add.b d1,d1          ; shift bit queue, high bit into carry
               bne .got_bit       ; queue not empty, bits remain
